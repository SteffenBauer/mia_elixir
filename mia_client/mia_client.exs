...
defmodule UDP do
  ...
  def send_msg(socket, msg) do
    :gen_udp.send(socket, {192,168,2,105}, 9000, msg)
  end
...
defmodule MiaClient do
  ...
  def start(reg_name \\ "MiaClient") do
    IO.puts "Start Mia Client as #{reg_name}"
    socket = UDP.open()
  ...
  def play_game(socket, current_bet) do
    case UDP.receiver do
      ["ROUND STARTING", token]         -> IO.puts "Round started, my token is #{token}"
                                           UDP.send_msg socket, "JOIN;#{token}"
                                           play_game socket, nil
      ["ROUND STARTED", runde, spieler] -> IO.puts "New round no. #{runde} with players #{spieler}"
                                           play_game socket, nil
      ["ROUND CANCELED", reason]        -> IO.puts "Round was canceled for reason #{reason}"
                                           play_game socket, nil
      ["YOUR TURN", token]              -> IO.puts "#{IO.ANSI.green}It is my turn!#{IO.ANSI.reset}"
                                           my_turn socket, token, current_bet
                                           play_game socket, current_bet
      ["PLAYER ROLLS", name]            -> IO.puts "Player #{name} rolls the dice..."
                                           play_game socket, current_bet
      ["ANNOUNCED", name, dice]         -> IO.puts "Player #{name} announced a roll of #{dice}"
                                           play_game socket, dice

      ["PLAYER WANTS TO SEE", name]     -> IO.puts "Player #{name} wants to see"
                                           play_game socket, current_bet
      ["ACTUAL DICE", dice]             -> IO.puts "Dice are actually #{dice}"
                                           play_game socket, current_bet
      ["PLAYER LOST",names, reason]     -> IO.puts "Player #{names} lost for reason #{reason}"
                                           play_game socket, nil
      ["SCORE", spielerpunkte]          -> IO.puts "Scores are #{spielerpunkte}"
                                           play_game socket, nil
  ...
  def handle_myroll(socket) do
    case UDP.receiver do
      ...
      ["PLAYER ROLLS", name]   -> IO.puts "I (#{name}) roll the dice..."
                                  handle_myroll socket
      _other                   -> nil
    end
  end
